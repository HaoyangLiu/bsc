...
import (
	...
	"github.com/ethereum/go-ethereum/cmd/token-bind-tool/config"
	constValue "github.com/ethereum/go-ethereum/cmd/token-bind-tool/const"
...
func DeployBEP20ContractTransferTotalSupplyAndOwnershipCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return TransferTokenAndOwnership(ethClient, keyStore, tempAccount, common.HexToAddress(config.LedgerAccount), contractAddr, chainId)
		},
	}
	cmd.Flags().String(constValue.KeystorePath, constValue.BindKeystore, "keystore path")
	cmd.Flags().String(constValue.ConfigPath, "", "config file path")
...
func ApproveBindAndTransferOwnershipAndRestBalanceBackToLedgerAccount(ethClient *ethclient.Client, keyStore *keystore.KeyStore, tempAccount accounts.Account, configData config.Config, bep20ContractAddr common.Address, peggyAmount *big.Int, chainId *big.Int) error {
	...
	tokenManagerInstance, err := tokenmanager.NewTokenmanager(tokenManagerContractAddr, ethClient)
	if err != nil {
		return err
	}
	...
	fmt.Println(fmt.Sprintf("Approve %s:%s to TokenManager from %s", lockAmount.String(), configData.Symbol, tempAccount.Address.String()))
	approveTxHash, err := bep20Instance.Approve(utils.GetTransactor(ethClient, keyStore, tempAccount, big.NewInt(0)), tokenManagerContractAddr, lockAmount)
	if err != nil {
		return err
	}
	...
	if restBEP20Balance.Cmp(big.NewInt(0)) > 0 {
		fmt.Println(fmt.Sprintf("Refund rest BEP20 balance %s to %s", restBEP20Balance.String(), configData.LedgerAccount))
		refundRestBEP20BalanceTxHash, err := bep20Instance.Transfer(utils.GetTransactor(ethClient, keyStore, tempAccount, big.NewInt(0)), common.HexToAddress(configData.LedgerAccount), restBEP20Balance)
		if err != nil {
			return err
		}
...
func ApproveBind(ethClient *ethclient.Client, ledgerWallet accounts.Wallet, ledgerAccount accounts.Account, bep2Symbol string, bep20ContractAddr common.Address, peggyAmount *big.Int, chainId *big.Int) error {
	...
	approveTxData, err := bep20ABI.Pack("approve", tokenManagerContractAddr, lockAmount)
	if err != nil {
		return err
	}
